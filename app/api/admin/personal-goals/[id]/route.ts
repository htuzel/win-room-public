// Win Room v2.0 - Admin Personal Goal Detail API
...
export async function PATCH(
  ...
  try {
    ...
    if (body.seller_id) {
      fields.push(`seller_id = $${fields.length + 1}`);
      values.push(body.seller_id);
    }
...
export async function DELETE(
  ...
  try {
    await query('DELETE FROM wr.personal_goals WHERE id = $1', [goalId]);
    return NextResponse.json({ success: true });
  } catch (error) {
