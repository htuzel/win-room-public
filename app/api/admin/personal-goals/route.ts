// Win Room v2.0 - Admin Personal Goals API
...
const VISIBILITY_SCOPES = new Set(['owner_only', 'admin_only']);
...
export async function GET(req: NextRequest) {
  ...
  const goals = await query(
    `
      SELECT id, seller_id, period_type, period_start, period_end,
             target_type, target_value, visibility_scope, created_at, updated_at
      FROM wr.personal_goals
      ${whereClause}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rows = await query(
      `
        INSERT INTO wr.personal_goals
          (seller_id, period_type, period_start, period_end, target_type, target_value, visibility_scope)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id
      `,
      [seller_id, period_type, period_start, period_end, target_type, numericTarget, visibility_scope]
    );
